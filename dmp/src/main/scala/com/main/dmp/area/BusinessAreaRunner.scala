package com.main.dmp.area

import ch.hsr.geohash.GeoHash
import com.main.dmp.etl.PmtETLRunner
import com.main.dmp.utils.HttpUtils
import com.main.dmp.utils.KuduHelper.{datasetToKuduContext, sparkToKuduContext}
...
//        .withColumn("area", lit(null))
    ...
    if (result != null) {
      ...
      spark.createKuduTable(AREA_TABLE_NAME, schema, keys, false)
  ...
  def fetchArea(longitude: Double, latitude: Double): String = {
    val jsonOption: Option[String] = HttpUtils.getAddressInfo(longitude, latitude)
    // Option[String] map -> Option[AMapLocation]
    jsonOption.map(json => HttpUtils.parseAddressInfo(json))
      .map(location => {
        val area = location.regeocode.addressComponent.businessAreas.getOrElse(List())
        // List[BusnessArea] -> 东单,西单,西二旗
        area.map(_.name).mkString(",")
      }).getOrElse("")
  }

  private val ODS_TABLE_NAME = PmtETLRunner.ODS_TABLE_NAME
