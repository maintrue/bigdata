...
object ShoppingGenderModel extends MultiSourceModel {
  ...
  override def process(sources: Array[DataFrame], fiveTags: Array[Tag], outFields: Array[String]): DataFrame = {
    ...
    val label = when('ogcolor.equalTo("樱花粉")
      ...
      .or('producttype.equalTo("吸尘器/除螨仪")), 1) // label=1 是女生
      .otherwise(0) // label=0 是男生
      .alias("gender")
    ...
    val featureIndexer = new VectorIndexer() // features 特征编码
    ...
    val decisionTree = new DecisionTreeClassifier()
      ...
      .setMaxDepth(5) // 最大树深度
      .setImpurity("gini") // 不重度 划分树的信息增益的东西
    ...
    val accEvaluator = new MulticlassClassificationEvaluator()
      .setPredictionCol("predict") // 预测值
      .setLabelCol("gender") // label列
      .setMetricName("accuracy") // 计算的方法 acc方法
