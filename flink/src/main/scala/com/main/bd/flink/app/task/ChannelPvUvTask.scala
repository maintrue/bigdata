package com.main.bd.flink.app.task

import com.main.bd.flink.app.bean.ClickLogWide
import com.main.bd.flink.app.util.{HBaseUtil, TimeUtil}
...
object ChannelPvUvTask {
  ...
  def process(clickLogWideDS: DataStream[ClickLogWide]) = {
    ...
    resultDS.addSink(new SinkFunction[ChannelRealPvUv] {
      override def invoke(value: ChannelRealPvUv, context: SinkFunction.Context[_]): Unit = {
        //查
